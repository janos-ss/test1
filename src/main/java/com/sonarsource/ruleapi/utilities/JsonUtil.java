...
public class JsonUtil {
  ...
  public static String toJson(Object object) {
    ...
    Gson gson = new GsonBuilder().disableHtmlEscaping().registerTypeAdapter(Instant.class,
        (JsonSerializer<Instant>) (instant, type, context) ->
  ...
  public static <T> T fromJson(String string, Class<? extends T> expectedClass) {
    ...
    Gson gson = new GsonBuilder().disableHtmlEscaping().registerTypeAdapter(Instant.class,
        (JsonDeserializer<Instant>) (json, type, jsonDeserializationContext) ->
            Instant.from(DateTimeFormatter.ISO_DATE_TIME.parse(json.getAsJsonPrimitive().getAsString()))
    ).create();
